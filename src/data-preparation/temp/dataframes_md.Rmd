...
```{r check results class change price, message=FALSE}
...
### Create dummy variable for neighbourhoods
```{r create dummy neighbourhood}
...
```
#### Check for the results
```{r check whether weeks in now of the class integer, include=FALSE, message=FALSE}
class(adam_reviews_pw$weeks)
```
```{r view the reviews data including weeknumbers}
View(adam_reviews_pw)
```
...
```{r overview reviews by neighbourhood per day}
...
```{r summarize reviews per week 2019}
...
```{r summarize reviews per week 2018}
